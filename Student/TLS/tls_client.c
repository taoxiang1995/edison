...
void* thread_func(void* argc)
{
    ...
    while(1) {
		...
		if(strstr(buf, "new_rate: ") != NULL) {
	    	...
	    	dprintf(log_file, "New rate %d received from server.\n", rate);
		}
		printf("Received message '%s' from server.\n\n", buf);
		dprintf(log_file, "Received message '%s' from server.\n\n", buf);
	}
	return NULL;
}
...
//	fprintf(log_file, "sasa");	
	...
	dprintf(log_file, "Current settings: rate: %d, range: %d\n", rate, range);
	heart_rate = 
	...
	dprintf(log_file, "Sending message '%s' to server...\n", buf);
